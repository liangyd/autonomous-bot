...
/*	if(bot_motion_state == RC){
	...
	float x_cur = world_odom.x;
	float y_cur = world_odom.y;

	//get target pos from command
	float x_next = path_list->path[path_idx].x;
	float y_next = path_list->path[path_idx].y;


	float err_dist = sqrt((x_next - x_cur)*(x_next - x_cur)   + (y_next - y_cur)*(y_next - y_cur));
...
/*		if( bot_motion_state == TURN){
		...
		if( bot_motion_state == RUN){
			...
			float x_prev,y_prev;
			if(path_idx == 0){
				x_prev = path_list_origin[0];
				y_prev = path_list_origin[1];
			}else{
				x_prev = path_list->path[path_idx - 1].x;
				y_prev = path_list->path[path_idx - 1].y;				
			}

			float d_deviation = (x_next - x_prev) * (y_cur - y_prev) - (y_next - y_prev) * (x_cur - x_prev);

			float d_dev_theta = PID_Cal(&path_devPID, d_deviation,freq_div);

			//result of atan func: -pi ~ pi. Dangerous range (pi + 1 = - pi -1)
			float target_theta = atan2(y_next - y_prev, x_next - x_prev) - d_dev_theta;
